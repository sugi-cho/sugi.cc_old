...
public class MassMeshes : MonoBehaviour
{
	...
	void Update ()
	{
		for (var i = 0; i < numDraw; i++) {
			mpb.Clear ();
			mpb.SetFloat ("_Offset", i * numInSingleMesh);
			Graphics.DrawMesh (mesh, transform.position, transform.rotation, drawMat, gameObject.layer, null, 0, mpb, castShadow, reserveShadow);
		}
	}
